 ...
 */
function blockAreas(lanes: Lane[], green: VillageModel['green']): number[] {
  const SNAP = 2;
...
for (const [pop, seed] of FIXTURES) {
  ...
  metricRows.push([
    ...
    (() => {
      const a = blockAreas(m.lanes, m.green);
      const mean = a.length === 0 ? 0 : a.reduce((x, y) => x + y, 0) / a.length;
      const med = a.length === 0 ? 0 : a[Math.floor(a.length / 2)];
      return `blocks ${a.length} mean ${Math.round(mean)} median ${Math.round(med)} m2`;
    })(),
