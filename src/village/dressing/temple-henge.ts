...
export function reserveTempleHenge(
  ...
  if (best) {
    const seed = Math.abs(Math.round(best.poi.position.x * 127 + best.poi.position.y * 311));
    const family = ['lintelled-ring','broken-ring','double-ring','horseshoe','oval-ring','ruined-ring'][seed % 6];
    const biome = artworkBiome(site.biome);
    best.poi.glyph = `sm-henge-${family}-${'abc'[Math.floor(seed / 6) % 3]}${biome === 'temperate' ? '' : `--${biome}`}`;
  }
  return best ?? null;
}
