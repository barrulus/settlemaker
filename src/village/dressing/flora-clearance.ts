...
export function floraClearance(
  ...
  water: Point[][], shorefrontReachM: number, reservations: Point[][], maxRadius = 0,
): (p: Point, radius?: number) => boolean {
  const cells = new Map<string, ((p: Point, radius?: number) => boolean)[]>();
  const size = 24;
  function add(points: Point[], padding: number, rejects: (p: Point, radius?: number) => boolean): void {
    if (!points.length) return;
    const minX = Math.floor((Math.min(...points.map(p => p.x)) - padding - maxRadius) / size);
    const maxX = Math.floor((Math.max(...points.map(p => p.x)) + padding + maxRadius) / size);
    const minY = Math.floor((Math.min(...points.map(p => p.y)) - padding - maxRadius) / size);
    const maxY = Math.floor((Math.max(...points.map(p => p.y)) + padding + maxRadius) / size);
  ...
  function polygon(poly: Point[], padding: number): void {
    add(poly, padding, (p, radius = 0) => pointInPolygon(p, poly) || poly.some((a, i) =>
      dist(p, closestPointOnSegment(p, a, poly[(i + 1) % poly.length])) <= Math.max(padding, radius)));
  }
  for (const lane of lanes) for (let i = 1; i < lane.points.length; i++) {
    ...
    add([a, b], clearance, (p, radius = 0) => dist(p, closestPointOnSegment(p, a, b)) <= clearance + radius);
  }
  for (const lot of lots) {
    ...
    add([new Point(obb.center.x - r, obb.center.y - r), new Point(obb.center.x + r, obb.center.y + r)], 0,
      (p, radius = 0) => pointInObb(p, obb, Math.max(1.5, radius)));
  }
  ...
  return (p, radius = 0) => {
    ...
    if (dist(p, green.centre) < greenDrawnRadius(green) + Math.max(1.5, radius)) return true;
    if (cells.get(`${Math.floor(p.x / size)},${Math.floor(p.y / size)}`)?.some(test => test(p, radius))) return true;
    for (const poly of water) {
      if (pointInPolygon(p, poly)) return true;

    }
    if (radius > 0 && waterBoundarySegments(water).some(([a, b]) =>
      dist(p, closestPointOnSegment(p, a, b)) < radius)) return true;
