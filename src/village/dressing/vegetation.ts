...
import {
  ...
  VEG_GLYPHS, VEG_INTERIOR_DENSITY, VEG_SCALE_MAX, VEG_SCALE_MIN,
} from '../constants.js';
...
export function buildVegetation(
  ...
  if (!(rimMaxM > 0)) return [];
  const rejected = floraClearance(green, lanes, lots, crofts, fields, site.water, shorefrontReachM, reservations);
  ...
  for (let x = -half; x <= half; x++) for (let y = -half; y <= half; y++) {
    ...
    trees.push({ id: `wood:${x}x${y}:0`, position, glyph: pickGlyph(site.biome, rng),
      scale: VEG_SCALE_MIN + rng.float() * (VEG_SCALE_MAX - VEG_SCALE_MIN) });
  }
  return trees;
}
