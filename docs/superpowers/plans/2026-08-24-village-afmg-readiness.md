...
- [x] Themed SVG via `render-theme.ts` (parchment default), honouring the `data-bg="paper"` rect the tiler's `cropSvgToTile` depends on. *(Done as the owner's option 1, and NOT via `render-theme.ts`: that module is the CITY renderer's theme and its parchment default would have re-skinned the village's gate-approved green. The village gets its own `src/village/theme.ts` instead, keyed on BIOME — the owner's reason, better than the plan's: the glyph set already resolves desert/tundra/tropical/coastal dwellings plus snow and mud materials, so a desert village was drawing sand houses on a temperate lawn. `TEMPERATE_THEME` carries the old constants exactly, pinned byte-identical by the Phase 2 hash. `renderVillage(model, pxPerMetre, theme?)` takes an override; themes can also override the symbol library's material tokens, which is what a night or snow scene needs. `data-bg="paper"` untouched.)*
